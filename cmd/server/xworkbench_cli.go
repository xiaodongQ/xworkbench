...
// handleXworkbenchCliReadmeMD serves the README.md file.
func (s *APIServer) handleXworkbenchCliReadmeMD(w http.ResponseWriter, r *http.Request) {
	data, err := os.ReadFile(filepath.Join("tools", "xworkbench-cli-skill", "README.md"))
	if err != nil {
		writeErr(w, http.StatusNotFound, "README.md not found")
		return
	}
	w.Header().Set("Content-Type", "text/plain; charset=utf-8")
	w.Write(data)
...
# 6. 下载 README.md
echo "==> 下载 README.md..."
curl -fsSL "$SERVER/api/xworkbench-cli/readme.md" -o "$SKILL_DIR/README.md" || {
    echo "WARNING: 下载 README.md 失败（不影响使用）"
}
