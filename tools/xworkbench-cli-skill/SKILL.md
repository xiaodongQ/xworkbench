...
xw_params:
  task_list: "列出手动任务 (--status 状态 --task-type local|remote --limit 数量)"
  task_create: "创建手动任务 (--title 标题 --description 描述 [--target-dir-id 远程目标DirShortcutID])"
  task_get: "获取手动任务详情 (task_id)"
  task_update: "更新手动任务 (task_id --title --description --status --priority [--target-dir-id])"
  task_run: "运行手动任务 (task_id)"
  task_cancel: "取消手动任务 (task_id)"
  task_delete: "删除手动任务 (task_id)"
